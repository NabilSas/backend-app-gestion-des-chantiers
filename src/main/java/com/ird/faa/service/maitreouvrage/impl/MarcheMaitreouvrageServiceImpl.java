...
import com.ird.faa.service.maitreouvrage.facade.MarcheMaitreouvrageService;
        ...
        import com.ird.faa.service.maitreouvrage.facade.MarcheEmployeMaitreouvrageService;
        import com.ird.faa.service.maitreouvrage.facade.EtatMarcheMaitreouvrageService;
        import com.ird.faa.service.maitreouvrage.facade.PaiementMarcheMaitreouvrageService;
...
@Service
public class MarcheMaitreouvrageServiceImpl extends AbstractServiceImpl<Marche> implements MarcheMaitreouvrageService {
...
private MarcheDao marcheDao;
    ...
    private ArchivableService<Marche> archivableService;
        @Autowired
        private LotMaitreouvrageService lotService ;
        @Autowired
        private MarcheEmployeMaitreouvrageService marcheEmployeService ;
        @Autowired
        private EtatMarcheMaitreouvrageService etatMarcheService ;
        @Autowired
        private PaiementMarcheMaitreouvrageService paiementMarcheService ;
        @Autowired
...
public Marche findByIdWithAssociatedList(Long id){
    ...
    @Override
    public Marche archiver(Marche marche) {
    if (marche.getArchive() == null) {
    marche.setArchive(false);
    }
    marche.setArchive(true);
    marche.setDateArchivage(new Date());
    marcheDao.save(marche);
    return marche;

    }

    @Override
    public Marche desarchiver(Marche marche) {
    if (marche.getArchive() == null) {
    marche.setArchive(false);
    }
    marche.setArchive(false);
